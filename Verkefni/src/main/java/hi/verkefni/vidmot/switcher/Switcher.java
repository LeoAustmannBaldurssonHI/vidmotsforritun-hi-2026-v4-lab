    ...
    public static void switchTo(View view, boolean isCache, Object hlutur) {
        ...
        try {
            ...
            if (cache.containsKey(view) && isCache) {
                ...
                FXMLLoader loader = new FXMLLoader(Switcher.class.getResource(view.getFileName()));
                root = loader.load();
            ...
            if (controller instanceof DataInterface) {
                ((DataInterface) controller).setGogn(hlutur);
            }
            scene.setRoot(root);
        } catch (IOException e) {
            e.printStackTrace();
